...
<div class="forum-features">
	...
	<% if Topics.MoreThanOnePage %>
		<p class="forum-pages-numbers">
			<% if Topics.PrevLink %><a class="button small icon fa=angle-double-left" href="$Topics.PrevLink"><% _t('Forum_ss.PREVLNK','Previous Page') %></a><% end_if %>
            ...
            <% if Topics.NextLink %><a class="button small icon fa-angle-double-right" href="$Topics.NextLink"><% _t('Forum_ss.NEXTLNK','Next Page') %> &gt;</a><% end_if %>
